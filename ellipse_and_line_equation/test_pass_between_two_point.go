...
func isWithinRange(x1, x2, x1_sol, x2_sol float64) bool {
	// 두 점 x1, x2의 최소값과 최대값 찾기
	xMin, xMax := math.Min(x1, x2), math.Max(x1, x2)
	// 해가 두 점 사이에 있는지 확인
	return (xMin <= x1_sol && x1_sol <= xMax) && (xMin <= x2_sol && x2_sol <= xMax)
}
...
func main() {
	...
	if D > 0 {
		...
		x1_sol := (-B + math.Sqrt(D)) / (2 * A)
		x2_sol := (-B - math.Sqrt(D)) / (2 * A)
		// 두 점 사이에서만 통과하는지 확인
		if isWithinRange(x1, x2, x1_sol, x2_sol) {
			fmt.Printf("Search Loc#1 [%d, %s, %f, %f] - p:(%d-%d)[%f, %f] - [%f, %f]\n", loc.Num, loc.Name, loc.Lat, loc.Lon, i+1, i+2, y1, x1, y2, x2)
			fmt.Println("직선이 두 점 사이에서만 타원을 통과합니다.")
		}
